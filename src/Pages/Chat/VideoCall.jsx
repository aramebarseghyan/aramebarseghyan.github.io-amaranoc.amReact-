...
export default function VideoCall({ mode, callId, caller, callee, onClose }) {
  ...
  // Նոր ֆունկցիա՝ զանգն ավարտելու և էջը թարմացնելու համար (X կոճակի համար)
  const endCall = async () => {
    await hangup();
  ...
  const rejectCall = async () => {
    ...
    await hangup();
    window.location.reload(); // Թարմացնում ենք էջը մերժելուց հետո
  };
  ...
  useEffect(() => {
    ...
    start().catch((error) => {
      ...
      alert(
        "Խնդրում ենք բրաուզերում թույլատրել տեսախցիկի և միկրոֆոնի հասանելիությունը:",
      );
      hangup();
    });
